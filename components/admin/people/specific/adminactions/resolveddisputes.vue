<template>
  <div id="dispute">
    <span id="order-title" v-if="isMobile">Resolved Dispute</span>
    <div v-if="!loading && !errorLoading">
      <div id="tanscation-cover" v-for="(item, index) in orders" :key="index">
        ...
        <span id="total" @click="linkTo(item.id)">
          Disputed date
          <span class="price">{{item.disputed_time}}</span>
        </span>
      </div>
    ...
    >Haven't Resolved Any Dispute</v-row>

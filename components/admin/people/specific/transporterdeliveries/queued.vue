<template>
  <div id="wrapper">
    <span id="comp-title" v-if="!loading && !errorLoading && Orders.length > 0">Queued For Delivery</span>
    <v-divider></v-divider>
    ...
    >
      <v-card elevation="0">No Queued Items For Delivery</v-card>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    openLink(id) {
      this.$emit('redirect', {
        url: '/admin/deliveries/queued/' + id,
        history: true
      })
    },

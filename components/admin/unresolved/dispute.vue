  ...
  <div id="live">
    ...
    <div v-if="!loading && !errorLoading">
      <div id="tanscation-cover" v-for="(item, index) in orders" :key="index">
        ...
        <span id="total" @click="linkTo(item.id)">
          Dispute date
          <span class="price">{{item.dispute_date}}</span>
        </span>
      </div>
...
export default {
  ...
  methods: {
    ...
    linkTo(id) {
      this.$emit('openUrl', {
        url: '/admin/unresolved/dispute/' + id,
        history: true
      })
    },
    getOrders() {

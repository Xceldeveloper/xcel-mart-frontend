      ...
      <div id="tanscation-cover" v-for="(item, index) in orders" :key="index">
        ...
        <span id="total">
         ...
         <span id="total" @click="linkTo(item.id)">
          Total Quantity
          <span class="price">{{formatToCurrency(item.total_quantity)}}</span>
        </span>
        <span id="total" @click="linkTo(item.id)">
          Total Amount
...
#tanscation-cover {
  ...
  box-shadow: 0px 2px 10px var(--shadow-color), -0px -0px 0px var(--shadow-color);
}

<template>
  <div id="wrapper">
    <span
      id="comp-title"
      v-if="!loading && !errorLoading && Orders.length > 0"
      ...
      <div id="delivered-items-wrapper" v-for="(item, index) in Orders" :key="index">
        <div id="delivered-container" @click="openLink(item.transaction_id)">
          ...
          <span id="item-deliver-date">
            Delivered Date
            <span class="value">{{item.transaction_delivered_date}}</span>
          </span>
        </div>
      </div>
    ...
    >
      <v-card elevation="0">You Have Not Completed Any Delivery...</v-card>
    </v-row>
  ...
  methods: {
     ...
     loadMore() {
      ...
      setTimeout(() => {
        this.Orders.push(
           {
            transaction_id: 234974,
            transaction_amount: 9022,
            transaction_payment_method: 'Cash on Delivery',
            transaction_order_date: '12/09/2020',
            transaction_delivered_date: '13/09/2020'
          },
          {
            transaction_id: 223456,
            transaction_amount: 34560,
            transaction_payment_method: 'Cash on Delivery',
            transaction_order_date: '12/09/2020',
            transaction_delivered_date: '13/09/2020'
          },
          {
            transaction_id: 23542,

<template>
  <div id="dispute">
    <span id="order-title" v-if="isMobile">Dispute</span>
    ...
    >No Have No Disputed Orders</v-row>
...
export default {
  ...
  methods: {
    getTotalOrders() {
      //get total disputed orders for customer from db
      this.totalOrders = 0
    },
    loadMore() {
      if (this.loadingMore) {
    ...
    linkTo(id) {
      this.$emit('redirect', {
        url: '/user/transaction/dispute/' + id,
        history: true
      })
    },
    getOrders() {

  ...
  <div id="transaction" v-cloak  v-if="CANRENDER">
   ...
   :root{
    ...
    <v-app-bar :color="Colors.accent" dense dark fixed>
      ...
      <v-toolbar-title>Canceled Delivery</v-toolbar-title>
    </v-app-bar>
    ...
    <div id="overall-wrapper" v-if="isAdminSignedIn && !isloading && !errorLoading">
      <div id="tanscation-cover">
        ...
        <span id="total">
          Canceled date
          <span class="price">{{canceledDate}}</span>
        </span>
        <span
          id="total"
          v-if="transporter !== ''"
        ...
        <v-expansion-panels class="mb-6" accordion elevation="0" v-if="canceledReason !== ''">
          <v-expansion-panel>
            <v-expansion-panel-header expand-icon="mdi-menu-down">Canceled Reason</v-expansion-panel-header>
            <v-expansion-panel-content>{{canceledReason}}</v-expansion-panel-content>
          </v-expansion-panel>
        </v-expansion-panels>
        <br />
        <span id="total">
    ...
    <div id="action-btn-cover" v-if="isAdminSignedIn && !isloading && !errorLoading">
      <v-btn
        @click="resolveCancelBottomSheet = true"
        rounded
        width="80%"
        max-width="500px"
        dark
        :color="Colors.accent"
      >Resolve Cancelation</v-btn>
    </div>
    ...
    <v-bottom-sheet
      ...
      max-width="500px"
      inset
      v-model="resolveCancelBottomSheet"
      :persistent="resolvingOrder"
    >
      <v-card>
        <v-card-text>
          <v-card-title>Resolve Cancelation</v-card-title>
          <v-divider></v-divider>
          <v-checkbox
          ...
          <v-checkbox
            :readonly="resolvingOrder"
            :color="Colors.accent"
            label="All Issues The customer or Transporter Complained About Has Been Resolved"
          ...
          <v-divider></v-divider>
          <v-btn
            @click="RESOLVE_DELIVERY_CANCELATION"
...
export default {
  ...
  data() {
    return {
      ...
      resolveCancelBottomSheet: false,
  ...
  methods: {
    ...
    generalCall() {
      ...
      let orderDetails = {
        ...
        transporter_id: 23,
        canceledReason: 'Could not find customer'
      }
    ...
    RESOLVE_DELIVERY_CANCELATION() {
      ...
      setTimeout(() => {
        this.resolveCancelBottomSheet = false
    ...
    beforeLeavingPage(to, from, next, callback) {
      ...
      if (this.resolveCancelBottomSheet) {

  ...
  <div id="transaction" v-cloak v-if="CANRENDER">
   ...
   :root{
    ...
    <v-app-bar :color="Colors.accent" dense dark fixed>
      ...
      <v-toolbar-title>Completed Transaction</v-toolbar-title>
      <v-spacer></v-spacer>
    ...
    <div id="overall-wrapper" v-if="isCutomerSignedIn && !isloading && !errorLoading">
      <div id="tanscation-cover">
        ...
        <span id="total">
          Delivered date
          <span class="price">{{deliveredDate}}</span>
        </span>
        <br />
        <span id="total">
      ...
      <v-btn dark :color="Colors.accent" @click="disputeDialog = true" block text>Open Dispute</v-btn>
      <br />
    </div>
    ...
    <v-bottom-sheet
      ...
      max-width="550px"
      width="95%"
      :persistent="openingDispute"
    >
      <v-card align="center" justify="center">
        <v-card-title>Dispute Transaction</v-card-title>
        <v-card-text>
          <v-textarea
            name="Dispute Reason"
            label="Dispute Reason"
            :color="Colors.accent"
            :readonly="openingDispute"
            outlined
            rows="1"
            auto-grow
            row-height="13"
...
export default {
  name: 'user-transaction-delivered-id',
  ...
  methods: {
    ...
    getOrderDetails(result) {
      ...
      this.deliveredDate = result.date_delivered
    ...
    openDistpute() {
      ...
      setTimeout(() => {
        ...
        this.snackMessage =
          'Transaction Has Been Disputed Please wait for Feedback'
        this.snackBar = true
        this.snackDurr = 2500
        setTimeout(() => {
          this.openNewPage('/', false)
